<form class="md:w-1/2 space-y-5" wire:submit.prevent="editVacant">
    ...
    <div>
        ...
        <select id="salary" wire:model="salary" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300
    ...
    <div>
        ...
        <select id="category" wire:model="category" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300
            focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600
            rounded-md shadow-sm">
    ...
    <div>
        ...
        <textarea class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300
            focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600
            rounded-md shadow-sm" wire:model="description" id="description" cols="30" rows="10"
            placeholder="Descripción general del puesto">
        </textarea>
        <x-input-error :messages="$errors->get('description')" class="mt-2" />
    </div>
    <div>
        <x-input-label for="new_image" class="mb-2" :value="__('Imagen Nueva')" />
        <input type="file" wire:model="new_image" id="new_image" class="block w-full text-sm text-slate-500
        ...
        <div class="my-5 w-80 mx-auto">
            <x-input-label class="mb-2" :value="__('Imagen Actual')" />
            <img src="{{ asset('storage/vacants') .'/'. $image }}" alt="{{ 'Imagen de la vacante ' . $title  }}">
        </div>


        <div class="my-5 w-80 mx-auto">
            @if ($new_image)
            <img src="{{ $new_image->temporaryUrl() }}" alt="Previsualizacion de la imagen de la vancante">
            @endif
        </div>
